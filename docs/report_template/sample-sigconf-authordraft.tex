...
\begin{itemize}
	...
	\item {\texttt{Applicable Diseases}}: A list of diseases for which the medicine is usually taken or applied.
	\item {\texttt{Diseases Information}}: A list of information on the medicine's applicable diseases.
	\item {\texttt{Possible Side Effects}}: A list of possible side effects provoked by the drug when taken.
	\item {\texttt{Excellent Review Percentage}}: Percentage of reviews with a rating superior to 7.
	\item {\texttt{Average Review Percentage}}: Percentage of reviews with a rating between 4 and 7.
        \item {\texttt{Poor Review Percentage}}: Percentage of reviews 
    with a rating inferior to 4.
        \item {\texttt{Review Average Rating}}: The average review score for 
    ...
    for producing a specific medicine.
        \item {\texttt{Manufacturer Description}}: A short description of 
    each company, containing some extra information.
        \item {\texttt{Manufacturer Start}}: The year the company was created.
        \item {\texttt{Manufacturer End}}: The year the company was shut 
    down, if available.
\end{itemize}

However, only a few of these document attributes are utilized in the retrieval process; the query is performed solely on the \texttt{Diseases Information}, \texttt{Reviews} and \texttt{Manufacturer Description} fields in the index, since these are attributes which primarily contain unstructured text.
...
\midrule
reviews & boostedText & \ding{51} & \ding{51}\\
diseases\_info & boostedText & \ding{51} & \ding{51}\\
manufacturer\_desc & boostedText & \ding{51} & \ding{55}\\
