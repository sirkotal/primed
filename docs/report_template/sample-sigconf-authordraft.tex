...
\begin{arrowlist}
    ...
    converts characters that don't belong
	\item \textbf{\textit{LowerCaseFilterFactory}:} Filter that converts all letters to lowercase.
	\item \textbf{\textit{SnowballPorterFilterFactory}:} Filter that applies stemming to tokens during text processing by generating pattern-based word stemmers - not as accurate as table-based stemmers, but faster and less complex.
\end{arrowlist}

Two additional field types, similar to \textbf{\textit{textBoosted}}, were created:

\begin{itemize}
        \item \textbf{\textit{diseasesBoosted}}: Very similar to 
    \textbf{\textit{textBoosted}}, but utilizes the \textbf{\textit{EnglishMinimalStemFilterFactory}} filter for stemming instead, since it's less powerful and, therefore, more suitable to handle fields with more technical terms.
