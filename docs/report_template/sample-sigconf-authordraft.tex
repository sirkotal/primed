...
Solr stores details about the field types and which fields it is expected to understand in a schema\cite{schema}. A schema allows for a specific search system to be configured on the indexing process. For system and performance evaluation purposes, a decision was made to create two distinct systems.
...
Despite not being part of the query parameters, other techniques are utilized via direct application to the user's query in the system:
\begin{itemize}
    \item \textbf{Term Boosts}: A specific set of terms relevant that may be relevant to the user are boosted individually when present in a query. The terms used specifically for our advanced system are present in table~[\ref{tab:boosted_terms}]
    \item \textbf{Fuzziness}: Enables approximate matching of terms, which makes it easier to handle typos or misspellings. For our advanced system, a decision was made to have the fuzziness value set to 1 and applied only to words with more than five letters.
\end{itemize}
...
Our QRELS were created using important information related to the expected queries.
...
\begin{arrowlist}
	\item Vomiting and cold treatment.
	\item Intacept reviews.
	\item Companies with treatment for headache.
	\item Best reviews from Novo Nordisk, Eli Lilly and Sanofi.
	\item Weight gain from antidepressants.
	\item Common side effects from Metoder tablet.
\end{arrowlist}
These queries were tested and evaluated. The outputs produced were quick and fast, having information easy to fetch. They were also found to have relevant information.
...
\begin{table}[H]
    ...
    \bottomrule
    \end{tabular}
    \caption{Boosted Terms}
    \label{tab:boosted_terms}
\end{table}
