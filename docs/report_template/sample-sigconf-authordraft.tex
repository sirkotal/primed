...
\begin{figure}[H]
  ...
  \Description{Polarity Scores Distribution}
...
\begin{table}[H]
    ...
    rqq & {!func}sum(product(reviews\_average\_rating, 4), product(polarity\_rating, 2))\\
    \bottomrule
    \end{tabular}}
...
The goal of the semantic search is to generate more accurate queries
...
The user interface was built using Rails\cite{rails}, which utilizes Ruby\cite{ruby} as the main language, with dependency management configured through Bundler\cite{bundler}. It can support Docker\cite{docker} containers, which would make an eventual deployment easier to due its controlled environment.

\subsubsection{Structure} 

The user interface structure is organized as follows:

\begin{itemize} 
    \item \textbf{app}: Contains the core logic of the application, such as controllers, models, and views. 
    \item \textbf{config}: Includes configuration files required for the application's operation. 
    \item \textbf{public}: Stores static assets such as images, CSS files, and JavaScript files. 
    \item \textbf{test}: Contains test cases for validating the website's functionality. 
    \item \textbf{vendor}: Stores external dependencies not managed directly by Bundler. 
\end{itemize}
...
\subsection{System Switch}

The user interface provides an easy way to switch between systems (schemas and query parameters), with options to select between the baseline, the advanced system, and the semantic search system.
To achieve this, however, it is required to be running multiple cores, so that the end user can quickly switch systems without requiring a reset of the Docker container.
