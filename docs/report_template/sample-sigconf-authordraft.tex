...
Each QRELS file was created manually; despite being a cumbersome process, it ensured that each file contained list of relevant documents for the specific information need, which was critical for evaluating the precision of each version of the search engine's retrieval capabilities.
...
The Mean Average Precision\cite{map}, or MAP, is a commonly used performance metric that calculates the mean of every relevant result is taken into account. Here, since there are too many results for each query, only the top 5 for each one were accounted, having for each query the averages

\subsubsection{P-R Curves}
