...
This data was also expanded with more data from the CUF page of Sicknesses from A-Z\cite{diseases_cuf_dataset}. The gathering of this data allowed for more information on our already existing diseases.

The main information extracted from this dataset is:
\begin{itemize}
	\item {\texttt{What is it?}}: A small description of the disease and what it is.
	\item {\texttt{Symptoms}}: Description of symptoms commonly related to that disease.
	\item {\texttt{Causes}}: Common causes for a person to gain that disease.
	\item {\texttt{Diagnosis}}: Information on how someone can diagnose the disease.
	\item {\texttt{Treatment}}: Common treatments for this disease, both pharmaceutical and/or surgeries.
	\item {\texttt{Prevention}}: How someone can prevent the disease.
\end{itemize}

This information contains about 556 entries, being available for public usage and information. It allowed for more understanding of our existing sicknesses and to complement missing information.
...
\end{table}
As we expected, the values of the P@k and MAP in the advanced
system (represented with an A) consistently outperformed or at
least matched those of the simple system (represented with an
S), showing there’s a clear advantage to using the boosted query
system.

\section{Frontend}

For the third milestone, we also developed a web-based frontend for the PRIMED search system.

\subsection{Frontend Technologies} The frontend uses Ruby as the main language, with dependency management configured through Bundler. It includes support for Docker containers, making deployment and execution in controlled environments easier.

\subsubsection{Frontend Structure} The directory structure is organized as described below:

\begin{itemize} \item \textbf{app}: Contains the core logic of the application, such as controllers, models, and views. \item \textbf{config}: Includes configuration files required for the application's operation. \item \textbf{public}: Stores static assets such as images, CSS files, and JavaScript files. \item \textbf{test}: Contains test cases for validating functionality. \item \textbf{vendor}: Stores external dependencies not managed directly by Bundler. \end{itemize}

\subsubsection{Configuration and Dependencies} The project dependencies are managed using the \texttt{Gemfile} and \texttt{Gemfile.lock} files, which define the libraries required for execution. Additionally, the \texttt{Dockerfile} and \texttt{.dockerignore} file configure the runtime environment for Docker containers.

\subsection{Schema Switching}
The frontend provides an easy and painless way to switch between schemas, with options to select between the simple schema, the advanced schema, and semantic search.
This is achieved by having multiple cores running, that a user can switch between via the interface.
...
\begin{figure}[H]
  ...
  \Description{Q1 P-R Curves -> Baseline System}
...
\begin{figure}[H]
  ...
  \Description{Q1 P-R Curves -> Advanced System}
...
\begin{figure}[H]
  ...
  \Description{Q2 P-R Curves -> Baseline System}
...
\begin{figure}[H]
  ...
  \Description{Q2 P-R Curves -> Advanced System}
...
\begin{figure}[H]
  ...
  \Description{Q3 P-R Curves -> Baseline System}
...
\begin{figure}[H]
  ...
  \Description{Q3 P-R Curves -> Advanced System}
...
\begin{figure}[H]
  ...
  \Description{Q4 P-R Curves -> Baseline System}
...
\begin{figure}[H]
  ...
  \Description{Q4 P-R Curves -> Advanced System}
